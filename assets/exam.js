...
(function () {
  ...
  function renderResult(correct) {
    ...
    // Build Your Own Exam supplies onChangeSelection so people can get back
    ...
    result.querySelector('[data-act="retake"]').addEventListener("click", () => {
      // Build Your Own Exam hands us onRetake so a retake reshuffles the
      // question order (and, for a custom count, redraws the subset) in
      // place instead of reloading the page and losing the picker's
      // checked topics/counts. Every other exam type on the site never
      // sets this, so they keep the plain reload — same as before.
      if (typeof data.onRetake === "function") data.onRetake();
      else location.reload();
    });
    const changeSelBtn = result.querySelector('[data-act="change-selection"]');
    if (changeSelBtn) changeSelBtn.addEventListener("click", () => data.onChangeSelection());
