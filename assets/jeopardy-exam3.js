/* ============================================================
   assets/jeopardy-exam3.js — Exam 3 Jeopardy game engine (3-player).

   Cloned from assets/jeopardy3.js (Exam 2's engine) rather than edited
   in place, so Exam 2's game is completely untouched by this file.
   ...
   file, so the Week 5/6 bank can be expanded or edited later without
   ...
   This also means the Week 2 game, Exam 2 game, and their data files are
   completely untouched by this page.

   Game shape, per the same spec Exam 2 was built to:
   - Each game randomly picks 3 of the Week 5 categories and 3 of the
     Week 6 categories from the bank, then shuffles the order of all 6
...
(function () {
  ...
  var PER_WEEK = 3; // categories drawn from each of Week 5 / Week 6
  ...
  // Picks 3 Week 5 + 3 Week 6 categories at random, then shuffles the
  // combined order so the board layout itself (not just the clues)
  // changes every game.
  function pickCategories() {
    var bank = bankCategories();
    var w5 = bank.filter(function (c) { return c.week === 5; });
    var w6 = bank.filter(function (c) { return c.week === 6; });
    var picked = shuffle(w5).slice(0, Math.min(PER_WEEK, w5.length))
      .concat(shuffle(w6).slice(0, Math.min(PER_WEEK, w6.length)));
